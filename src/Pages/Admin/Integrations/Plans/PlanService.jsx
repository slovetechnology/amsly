  ...
  return (
    <AdminLayout pagetitle="Automation Service">
      <div className="w-11/12 max-w-3xl mx-auto flex flex-col">
        ...
        {subs.map((item, i) => (
          item.category?.slice(0, 3) === id?.slice(0, 3) && item.locked === 'no' && <Link to={`/auth/admin/integration/plans/${id}/${item.id}`} key={i} className="bg-white p-4 text-center cursor-pointer uppercase rounded-lg mb-2">{item.network}</Link>
        ))}
      </div>
    </AdminLayout>
  )
