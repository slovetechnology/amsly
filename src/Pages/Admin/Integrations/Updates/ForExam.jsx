...
const ForExam = ({ data, exams }) => {
    const [isNew, setIsNew] = useState(true)
    const [works, setWorks] = useState(exams?.exnetworks || [])
    const [loading, setLoading] = useState(false)
    const [packid, setpackid] = useState('')
    const [forms, setForms] = useState({
        serviceName: exams?.serviceName || '',
        variationName: exams?.variationName || '',
        mobileName: exams?.mobileName || '',
        method: exams?.method || '',
        format: exams?.format || '',
    })
    ...
    const handleSubmission = async () => {
        ...
        if (!forms.variationName) return ToastAlert('Variation Name is required')
        ...
        setLoading(true)
        const res = await PostUrl(Api.subs.add_exam_automation, formdata)
        setLoading(false)
        if(res.status === 200) {
            ...
            <div className="w-full bg-white p-4">
                <div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div className="mb-4">
                            <div className="text-slate-500">Variation Name</div>
                            <input name="variationName" value={forms.variationName} onChange={handleForms} type="text" className="input" />
                        </div>
                        <div className="mb-4">
                    ...
                    <div className="">
                        ...
                        {works.length > 0 ? works.map((item, i) => (
                           item.nets === 'exam' && <SingleApiNetwork
                                item={item}
                                key={i}
                ...
                <div className="p-4 w-fit ml-auto">
                    <button onClick={handleSubmission} className="bg-indigo-600 py-3 uppercase px-6 rounded-full text-white">integrate exam Api</button>
                </div>
            </div>
        </div>
    )
