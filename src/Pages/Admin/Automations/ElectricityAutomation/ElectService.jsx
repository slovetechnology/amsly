...
const ElectService = () => {
  ...
  const handleDuplicates = () => {
    ...
    return subs.map((item, i) => (
      item.category.startsWith(`electricity`) && item.locked === 'no' &&
      <Link to={`/auth/admin/automation/electricity/${item.tag || slugify(item.network, {lower: true})}`} key={i} className="bg-white p-4 text-center cursor-pointer uppercase rounded-lg mb-2">{item.tag || item.network}</Link>
    ))
  }
